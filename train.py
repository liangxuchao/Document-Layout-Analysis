...
from tools import register_dataset
...
def custom_mapper(dataset_dict):
    ...
    transform_list = [
        T.RandomApply(T.RandomFlip(prob=0.5, horizontal=True, vertical=False),prob=0.5)
        ,T.RandomApply(T.RandomRotation(angle=[-15,15], expand=True, center=None, sample_style="range", interp=None), 
                      prob=0.3)
        ...
        ,T.RandomApply(T.RandomLighting(0.7),prob=0.5)
...
def visualize(args):
    ...
    cfg = setup(args)
    train_data_loader = CustomTrainer.build_train_loader(cfg)
    data_iter = iter(train_data_loader)
    batch = next(data_iter)

    rows, cols = 2, 2
    plt.figure(figsize=(1025,1025))
